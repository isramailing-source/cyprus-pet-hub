    ...
    for (let i = 0; i < articlesToGenerate; i++) {
      try {
        ...
        const articleHtml = await generateArticleWithAssistant({
          ...
          lengthWords: 8000,
          keywords: randomTopic.focus ? [randomTopic.focus] : [],
          additionalInstructions: `Focus on ${randomTopic.focus} and provide practical advice for pet owners.`,
          targetAudience: 'pet owners and enthusiasts'
        });
        
        // Parse the HTML to extract title and create article data structure
        const titleMatch = articleHtml.match(/<h1[^>]*>(.*?)<\/h1>/i);
        const title = titleMatch ? titleMatch[1].replace(/<[^>]*>/g, '') : randomTopic.title;
        
        // Create excerpt from first paragraph
        const paragraphMatch = articleHtml.match(/<p[^>]*>(.*?)<\/p>/i);
        const excerpt = paragraphMatch ? 
          paragraphMatch[1].replace(/<[^>]*>/g, '').substring(0, 200) + '...' : 
          'Expert advice on pet behavior and training techniques.';
        
        // Validate article content
        if (!articleHtml || articleHtml.length < 500) {
          throw new Error('Generated article content is too short or empty')
        }
        ...
        const articleToInsert = {
          ...
          excerpt: excerpt,
          slug: uniqueSlug,
          meta_title: `${title} - Cyprus Pets Expert Guide`,
          meta_description: excerpt.substring(0, 160),
          tags: [randomTopic.category.toLowerCase(), 'pet training', 'behavior', 'expert advice'],
...
Do not add any disclaimers or signatures.`;
  
  // Call Gemini API to generate the article
  try {
